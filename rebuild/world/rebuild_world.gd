...
func _build_mountain_highway(points: Array[Vector3]) -> void:
	...
	highway.mesh = mesh
	add_child(highway)
	# One continuous concave collision shape follows the exact asphalt mesh.
	# The old chain of rotated support boxes overlapped at bends, producing the
	# invisible transverse steps that stopped cars even on clear-looking road.
	var road_body := StaticBody3D.new()
	road_body.name = "ContinuousMountainHighwayCollision"
	road_body.set_meta("surface", "concrete")
	var road_collision := CollisionShape3D.new()
	road_collision.shape = mesh.create_trimesh_shape()
	road_body.add_child(road_collision)
	highway.add_child(road_body)
...
func _build_dense_downtown_boulevard() -> void:
	## Build a recognizable downtown street wall instead of another sparse grid.
	## Two continuous rows face the z=-480 avenue, with only 62 metres between
	## addresses. Storefronts, apartment-like commercial blocks and skyscrapers
	## alternate so the skyline has both density and variety. Every façade still
	## uses the standard enterable, furnished interior system.
	var frontage_x: Array[float] = []
	for x in range(-840, 421, 63):
		frontage_x.append(float(x))
	var frontage_z := [-446.0, -514.0]
	var models := ["building-a.glb", "building-d.glb", "building-g.glb",
			"building-j.glb", "building-m.glb", "building-skyscraper-a.glb",
			"building-b.glb", "building-h.glb", "building-skyscraper-c.glb"]
	var lot_id := 950
	for row in frontage_z.size():
		for column in frontage_x.size():
			_build_enterable_lot(COMMERCIAL_ROOT + "/" \
					+ models[(column + row * 3) % models.size()],
					Vector3(frontage_x[column], 0.0, frontage_z[row]),
					0.0 if row == 0 else PI, 5.8, lot_id)
			lot_id += 1
	# Wide continuous sidewalks connect every entrance along the avenue.
	for edge_z in [-493.5, -466.5]:
		_box(Vector3(1320, 0.055, 7.0), Vector3(-210, 0.027, edge_z),
				_sidewalk_mat, false, "concrete")
