...
func _build_mesh() -> void:
	...
	cloth.albedo_color = body_color.darkened(0.35)
	cloth.roughness = 1.0
	...
	for data in [[-0.34, true], [0.34, false]]:
		var pivot := Node3D.new()
		pivot.position = Vector3(data[0], 1.45, 0)
		var arm := MeshInstance3D.new()
		var amesh := BoxMesh.new()
		amesh.size = Vector3(0.11, 0.62, 0.13)
		amesh.material = skin
		arm.mesh = amesh
		arm.position = Vector3(0, -0.31, 0)
		pivot.add_child(arm)
		# Zombie arms hang forward slightly
	...
	for lx in [-0.12, 0.12]:
		...
		lmesh.size = Vector3(0.14, 0.68, 0.16)
		lmesh.material = cloth
		leg.mesh = lmesh
		leg.position = Vector3(lx, 0.34, 0)
