...
const ONE_SHOTS := ["Punch_Cross", "Punch_Jab", "Hit_Chest", "Hit_Head", "Death01"]
...
func _build_mesh() -> void:
	## Animated mannequin rig (same CC0 pack as the player), tinted per kind.
	_mesh_root = Node3D.new()
	_mesh_root.scale = Vector3.ONE * size_mult
	add_child(_mesh_root)
	var rig: Node3D = BodyScene.instantiate()
	rig.rotation.y = PI  # model faces +Z; zombies walk toward -Z
	...
	if mesh:
		...
		skin.albedo_color = body_color.darkened(0.08)
		skin.roughness = 0.95
		mesh.material_override = skin
	# Glowing eyes on the head bone so they read as undead at night
	...
	if skel:
		...
		for ex in [-0.055, 0.055]:
			...
			var box := BoxMesh.new()
			box.size = Vector3(0.045, 0.04, 0.02)
			box.material = eye_mat
			mi.mesh = box
			mi.position = Vector3(ex, 0.09, 0.115)
			att.add_child(mi)
	if _anim:
		_anim.play("Idle")


func _play(anim: String, blend := 0.25, speed := 1.0) -> void:
	if _anim == null:
		return
	if _anim_state == anim:
...
func _play_one_shot(anim: String, speed := 1.0) -> void:
	if _anim == null:
		return
	_anim_state = anim
	_anim.speed_scale = speed
...
func _do_attack(delta: float) -> void:
	...
	if _windup >= 0.35:
		...
		_voice.play()
		_punch_flip = not _punch_flip
		_play_one_shot("Punch_Jab" if _punch_flip else "Punch_Cross", 1.4)
...
func _animate(_delta: float) -> void:
	if _anim == null or state == State.DEAD:
		return
	# Let punches and hit reactions finish before locomotion resumes
	if _anim_state in ONE_SHOTS and _anim.is_playing():
		return
	...
	if hspeed > 2.2:
		_play("Jog_Fwd", 0.25, clampf(hspeed / 3.0, 0.7, 1.5))
	elif hspeed > 0.25:
		_play("Walk", 0.25, clampf(hspeed / 1.4, 0.55, 1.4))
	else:
		_play("Idle", 0.3, 0.75)
...
func _die() -> void:
	...
	_voice.pitch_scale = randf_range(0.9, 1.1)
	_voice.play()
	_play_one_shot("Death01")
	var tween := create_tween()
	tween.tween_interval(6.5)
	tween.tween_property(_mesh_root, "position:y", -1.6, 2.0)
